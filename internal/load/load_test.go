...
func TestProvisionLoads(t *testing.T) {
	...
	p, err := load.Provision(filepath.Join(repoRoot, "internal/testdata/dsv/provision.yaml"))
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func TestFullProvisionLoads(t *testing.T) {
	repoRoot, _ := filepath.Abs("../..")
	fp, err := load.FullProvision(filepath.Join(repoRoot, "internal/testdata/dsv/full-provision-filled.yaml"))
	if err != nil {
		t.Fatalf("load: %v", err)
	}
...
func TestDetectKind(t *testing.T) {
	repoRoot, _ := filepath.Abs("../..")
	tm, err := load.DetectKind(filepath.Join(repoRoot, "internal/testdata/dsv/provision.yaml"))
	if err != nil {
...
func TestProvisionResolvedNoExtends(t *testing.T) {
	repoRoot, _ := filepath.Abs("../..")
	path := filepath.Join(repoRoot, "internal/testdata/dsv/provision.yaml")
